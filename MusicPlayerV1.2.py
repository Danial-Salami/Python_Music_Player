...
class MusicPlayerGUI:
    def __init__(self, root):
        ...
        play_img = Image.open("icons/211876_play_icon.png")
        play_img = play_img.resize((32, 32))
        self.play_icon = ImageTk.PhotoImage(play_img)
        pause_img = Image.open("icons/211871_pause_icon.png")
        pause_img = pause_img.resize((32, 32))
        self.pause_icon = ImageTk.PhotoImage(pause_img)
        unpause_img = Image.open("icons/9044200_continue_filled_icon.png")
        unpause_img = unpause_img.resize((32, 32))
        self.unpause_icon = ImageTk.PhotoImage(unpause_img)
        stop_img = Image.open("icons/216325_media_stop_icon.png")
        stop_img = stop_img.resize((32, 32))
        self.stop_icon = ImageTk.PhotoImage(stop_img)
        next_img = Image.open("icons/forward-button.png")
        next_img = next_img.resize((32, 32))
        self.next_icon = ImageTk.PhotoImage(next_img)
        previous_img = Image.open("icons/rewind-button.png")
        previous_img = previous_img.resize((32, 32))
        self.previous_icon = ImageTk.PhotoImage(previous_img)
        volume_img = Image.open("icons/9004796_speaker_sound_music_audio_volume_icon.png")
        volume_img = volume_img.resize((12, 12))
        ...
        self.volume_slider = tk.Scale(button_frame, from_=0, to=100, orient=tk.HORIZONTAL, command=self.set_volume,
                                      font=("Arial", 10), bg=self.primary_color, fg=self.secondary_color, highlightbackground=self.primary_color)
